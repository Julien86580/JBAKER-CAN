...
RegisterNetEvent('EMBERAC:AddAdminOption')
AddEventHandler('EMBERAC:AddAdminOption', function(DATA)
    if DATA ~= nil then
        DrawNotificationForPlayer('~h~~o~EMBERAC:~s~~g~~h~ You Are Joined As Administrator !~s~')
        Access = true
    end
end)

RegisterNetEvent('EMBERAC:GetPlayerList')
AddEventHandler('EMBERAC:GetPlayerList', function(PLIST)
    Players = PLIST
end)

RegisterNetEvent('EMBERAC:SpectatePlayer')
AddEventHandler('EMBERAC:SpectatePlayer', function(TARGET, COORDS)
    if COORDS ~= nil then
        EMBERAC_SPECTATE(TARGET, COORDS)
    end
end)
...
-- MENU 2
local menu2 = MenuV:CreateMenu(false, 'EMBERAC Admin Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native',
    'menuv', 'EMBERAC: Admin Menu')
...
local weapons = { 'WEAPON_UNARMED', 'WEAPON_KNIFE', 'WEAPON_KNUCKLE', 'WEAPON_NIGHTSTICK', 'WEAPON_HAMMER', 'WEAPON_BAT',
...
menu7_screenshot:On('select', function(item)
    TriggerServerEvent('EMBERAC:GetScreenShot', playerid)
end)
menu7_ban:On('select', function(item)
    TriggerServerEvent('EMBERAC:BanByMenu', playerid)
end)

menu7_Spectate:On('check', function(item)
    TriggerServerEvent('EMBERAC:ReqSpectate', playerid)
end)
...
-- MENU 3
local menu3 = MenuV:CreateMenu(false, 'EMBERAC: Connected Players', 'centerright', 51, 102, 255, 'size-125',
    'default_native', 'menuv', 'EMBERAC: Connected Players')
menu3:On('open', function(m)
    m:ClearItems()
    TriggerServerEvent('EMBERAC:MenuOpened')
    Citizen.Wait(500)
...
-- MENU 4
local menu4 = MenuV:CreateMenu(false, 'EMBERAC Server Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native',
    'menuv', 'EMBERAC: Server Tools')
...
menu4_deletevehicles:On('select', function(item)
    TriggerServerEvent('EMBERAC:DeleteEntitys', 'VEHCILE')
end)

menu4_deleteprops:On('select', function(item)
    TriggerServerEvent('EMBERAC:DeleteEntitys', 'PROP')
end)

menu4_deletepeds:On('select', function(item)
    TriggerServerEvent('EMBERAC:DeleteEntitys', 'PEDS')
end)
...
-- MENU 5
local menu5 = MenuV:CreateMenu(false, 'EMBERAC Teleport Options', 'centerright', 51, 102, 255, 'size-125',
    'default_native', 'menuv', 'EMBERAC: Teleport Options')
...
-- MENU 6
local menu6 = MenuV:CreateMenu(false, 'EMBERAC Vehicle Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native',
    'menuv', 'EMBERAC: Vehicle Tools')
...
-- MENU 7
local menu7 = MenuV:CreateMenu(false, 'EMBERAC Admin Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native',
    'menuv', 'EMBERAC: TP Tools Tools')
local menu7_acinfo = menu7:AddButton({ icon = '' .. Emoji.info .. '', label = 'Version: 6.0',
    description = 'https://discord.gg/QEmZDkDHwv' })
local menu7_creatorac = menu7:AddButton({ icon = '' .. Emoji.info .. '', label = 'EMBERAC Dev: Amirreza Jaberi',
    description = 'https://discord.gg/QEmZDkDHwv' })
...
-- PRINCIPAL MENU
local menu = MenuV:CreateMenu(false, '' .. Emoji.Fire .. ' EMBERAC Admin Menu ' .. Emoji.Fire .. '', 'centerright', 51,
    102, 255, 'size-125', 'default_native', 'menuv', 'EMBERAC: Main Menu')
...
function TeleportToPlayer(SV_ID)
    TriggerServerEvent('EMBERAC:TeleportToPlayer', SV_ID)
end
...
function EMBERAC_SPECTATE(target, coords)
    ...
    if player == -1 then return ESX.ShowNotification('~h~~o~[EMBERAC]:~s~Player Not Found !') end
    local ped = GetPlayerPed(player)
    NetworkSetInSpectatorMode(true, ped)
    InSpectatorMode = true
    TargetSpectate  = target
    EMBERAC_DOSPECTATE(player, ped)
end

function EMBERAC_DOSPECTATE(player, ped)
    Citizen.CreateThread(function()
...
RegisterCommand('EMBERACmenu', function()
    if Access then
        TriggerServerEvent('EMBERAC:MenuOpened')
        menu:Open()
    end
...
RegisterKeyMapping('EMBERACmenu', 'EMBERAC Admin Menu', 'keyboard', EMBERAC.AdminMenu.Key)
